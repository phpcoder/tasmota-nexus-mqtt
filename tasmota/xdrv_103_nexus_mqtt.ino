...
void NexusToMqttProcessing(void)
{
  ...
  if (int_enable == 0) {
    ...
    if (check_nexus_data() == 0) {
      ...
      snprintf_P(topic, topic_size, PSTR("test/sensor/NEXUS_%d/state"), nexus_data_prev.ID);


      snprintf_P(payload, payload_size, 
        PSTR("{\"" D_JSON_TIME "\":\"%s\",\"proto\":\"NEXUS\",\"id\":\"%d\",\"temp\":\"%s\",\"hum\":\"%d\"}"), 
